...
export const DEFAULT_PROVIDERS: Omit<IProvider, 'id' | 'apiKey'>[] = [
  ...
  },
  {
    ...
    model: ['agnes-video-v2.0', 'agnes-1.5-flash', 'agnes-2.0-flash', 'agnes-image-2.0-flash', 'agnes-image-2.1-flash'],
    capabilities: ['text', 'vision', 'image_generation', 'video_generation'],
    headers: {
    ...
    videoQueryEndpoint: '/agnesapi?video_id={{VIDEO_ID}}',
  },
];
...
export function classifyModelByName(modelName: string): ModelCapability[] {
  ...
  const videoPatterns = [
    ...
    'agnes-video', 'cogvideo', 'hunyuan-video', 'minimax-video', 'hailuo', 'pika',
  ...
  const imageGenPatterns = [
    ...
    'playground', 'recraft', 'kolors', 'agnes-image', 'seedream',
  ];
  ...
  if (name.includes('agnes')) {
    if (name.includes('video')) return ['text', 'vision', 'video_generation'];
    if (name.includes('image')) return ['text', 'vision', 'image_generation'];
    return ['text', 'vision'];
  }
